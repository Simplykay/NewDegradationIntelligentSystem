...
pages = [
    ...
    ("03", "Field Intelligence",      "How does irrigation and heat accumulation affect CT?",
     COLORS["accent_blue"]),
    ("04", "ShelfSight",              "How many seasons until this lot's CT drops below 60%?  [PRIMARY]",
     COLORS["accent_blue"]),
    ("05", "Pipeline Stage Analysis", "At which stage is quality gained or lost most?",
     COLORS["accent_amber"]),
    ("06", "Maturity Band Profiler",  "Which Relative Maturity bands perform best?",
     COLORS["accent_teal"]),
...
findings = [
    ...
    ("Arizona is highest-risk region at ~29.4%",
    ...
    ("Median lot has 2-3 growing seasons of viable shelf-life remaining",
     "ShelfSight (Weibull AFT) provides per-lot precision — critical for sell-by scheduling."),
]
...
# ── Section C: The Four Models ─────────────────────────────────────
st.markdown("---")
st.markdown("### The Four Models — Plain English")

models_info = [
    (
        "LotGuard — Risk Scorer",
        COLORS["accent_amber"],
        "Looks at seed quality measurements and weather data and answers: what is the probability "
        "this lot is degraded right now? Ranks every lot from 0% to 100% risk so you know exactly "
        "which ones to prioritise.",
        "Risk probability 0–100% that CT score is below 60.",
        "AUC = 1.00 on test set — correctly ranks every lot by degradation risk.",
        False,
    ),
    (
        "QualityScope — CT Score Predictor",
        COLORS["accent_teal"],
        "Predicts the actual cool test (CT) score a lot will receive — a specific number, not just "
        "pass or fail. Gives buyers and warehouse managers a precise quality figure to work with.",
        "Predicted CT score on a 0–100 scale.",
        "RMSE = 0.87 points — within a single lab measurement of the real result.",
        False,
    ),
    (
        "GradeView — Quality Grader",
        COLORS["accent_green"],
        "Classifies each lot into one of three clear business tiers: Degraded, At Risk, or High "
        "Quality. Trained with class weighting so the boundaries are reliable even for rare "
        "degraded lots.",
        "One of three quality grades: Degraded · At Risk · High Quality.",
        "Macro-F1 = 1.00 on test set — accurate across all three grades.",
        False,
    ),
    (
        "ShelfSight — Shelf-Life Predictor",
        COLORS["accent_blue"],
        "The primary business deliverable. Asks: how many growing seasons before this specific lot "
        "becomes too degraded to sell? Uses survival analysis — the same mathematical framework "
        "hospitals use to predict patient outcomes — applied to seed storage. Every lot gets its "
        "own personalised sell-by timeline.",
        "Median seasons-until-degradation per lot, with confidence interval.",
        "C-index = 0.93 — correctly ranks lots by shelf-life remaining.",
        True,
    ),
]

for name, color, what, output, metric, is_primary in models_info:
    border = f"2px solid {COLORS['accent_blue']}" if is_primary else f"1px solid {COLORS['border_subtle']}"
    glow   = f"box-shadow: 0 0 18px {COLORS['accent_blue']}30;" if is_primary else ""
    with st.expander(f"{'⭐ ' if is_primary else ''}{name}", expanded=is_primary):
        st.markdown(f"""
        <div style="border-left:4px solid {color};padding-left:12px;margin-bottom:8px">
            <b>What it does:</b> {what}
        </div>
        """, unsafe_allow_html=True)
        c1, c2 = st.columns(2)
        c1.markdown(f"**Output:** {output}")
        c2.markdown(f"**Performance:** {metric}")
...
challenges = [
    ("Cleanout_PCT corrupted",
     "Values ranged from -92,618 to 3,512. Column dropped entirely."),
    ("915 CT > WG anomalies",
     "Biologically impossible. Excluded from training as data capture failures."),
    ("New varieties every season",
     "Solved by replacing Variety with rm (Relative Maturity) — stable across seasons "
     "and available for any variety including new ones."),
    ("2026 field operations data",
     "Not yet available for all lots. Pipeline runs cleanly; weather join logs coverage "
     "at startup. No waiting required."),
    ("Sparse FFA coverage (7.3%)",
     "Used as supplementary signal only, not primary feature."),
    ("Class imbalance",
     "Handled with scale_pos_weight in XGBoost models."),
]
...
recs = {
    ...
    "Short-term": [
        "Deploy ShelfSight (Page 04) as the primary inventory management tool.",
        "Review high-risk variety contracts using the regional risk map.",
        "When 2026 field operations data arrives, re-run the weather enrichment step.",
    ],
    "Medium-term": [
...
glossary = {
    ...
    "GradeView":              "The quality grading model — classifies lots into Degraded, At Risk, or High Quality.",
    "Hazard Ratio":           "Cox PH output; >1 increases risk, <1 protective.",
    "Kaplan-Meier Curve":     "Survival probability over time.",
    "LightGBM":               "Fast gradient boosting; used by QualityScope.",
    "Lineage Dataset":        "Primary quality database with 70,000+ records.",
    "LotGuard":               "The risk scoring model — gives a 0–100% degradation probability for each lot.",
    "Lot":                    "Single trackable batch of seed.",
    "QualityScope":           "The CT score prediction model — gives a precise numeric quality score (0–100).",
    ...
    "ShelfSight":             "The primary deliverable — predicts how many seasons a lot has before CT drops below 60.",
    ...
    "Weibull AFT":            "Survival model variant used by ShelfSight to predict direct shelf-life per lot.",
    "XGBoost":                "Gradient boosting algorithm; used by LotGuard and GradeView.",
}
